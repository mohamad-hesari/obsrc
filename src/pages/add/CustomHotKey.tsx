...
function getKeyName(key: string) {
  return key.replace("OBS_KEY_", "");
}

const CustomHotKey = withLayout(
  function HotKeys() {
    ...
    function toggle(set: React.Dispatch<React.SetStateAction<boolean>>) {
    ...
    const buttons = [
      { label: "Shift", state: shift, setState: setShift },
      { label: "CTRL", state: ctrl, setState: setCtrl },
      { label: "ALT", state: alt, setState: setAlt },
      { label: "CMD (Mac)", state: command, setState: setCommand },
    ];

    return (
      <div className="divide-y divide-gray-400 [&>*]:rounded-none [&>*]:px-0 [&>*]:py-2">
        {buttons.map(({ label, state, setState }) => (
          <Button
            key={label}
            color="glasses"
            type="button"
